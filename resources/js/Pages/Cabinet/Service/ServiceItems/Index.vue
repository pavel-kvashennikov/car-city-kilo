...
<template>
    <CabinetLayout>
        <div class="flex items-center justify-between mb-6">
            ...
            <Link href="/cabinet/service/service-items/create" class="bg-blue-600 text-white px-4 py-2 rounded-lg text-sm hover:bg-blue-700">
                Добавить услугу
            </Link>
        </div>
        <DataTable :columns="columns" :rows="serviceItems">
            <template #cell-price="{ value }">{{ formatPrice(value) }}</template>
            <template #cell-duration_minutes="{ value }">{{ value }} мин</template>
            <template #cell-actions="{ row }">
                <Link :href="`/cabinet/service/service-items/${row.id}/edit`" class="text-blue-600 text-sm hover:underline">Ред.</Link>
            </template>
        </DataTable>
    </CabinetLayout>
</template>
